...
class Mod_lap_masuk extends CI_Model
{
	...
	{ 
		$query = $this->db->query("
			SELECT
				tv.*, tmd.keterangan, tmd.qty, ts.nama as nama_satuan, tud.nama_lengkap_user
			FROM
				tbl_verifikasi tv
				JOIN tbl_trans_masuk_detail tmd on tv.id_in_detail = tmd.id
				JOIN tbl_satuan ts on tmd.satuan = ts.id
