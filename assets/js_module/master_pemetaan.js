...
$(document).ready(function() {
	...
	table = $('#tabel_index').DataTable({
        ...
        serverSide: true,
		ajax: {
			url  : base_url + "master_pemetaan/list_pemetaan",
			type : "POST" 
		},
...
function add_pemetaan()
{
    reset_modal_form();
    save_method = 'add';
	$('#modal_form').modal('show');
	$('#modal_title').text('Tambah Pemetaan Baru'); 
}

function edit_pemetaan(id)
{
    reset_modal_form();
    save_method = 'update';
    //Ajax Load data from ajax
    $.ajax({
        url : base_url + 'master_pemetaan/edit_pemetaan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            $('[name="id_pemetaan"]').val(data.old_data.id);
            $('[name="keterangan"]').val(data.old_data.keterangan);
            $('[name="umur_awal"]').val(data.old_data.umur_awal);
            $('[name="umur_akhir"]').val(data.old_data.umur_akhir);
            $('#modal_form').modal('show');
	        $('#modal_title').text('Edit Data Pemetaan'); 
        },
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_pemetaan/add_data_pemetaan';
        txtAksi = 'Tambah Pemetaan';
    }else{
        url = base_url + 'master_pemetaan/update_data_pemetaan';
        txtAksi = 'Edit Pemetaan';
    }
...
function delete_pemetaan(id){
    swalConfirmDelete.fire({
        title: 'Hapus Data Pemetaan ?',
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_pemetaan/delete_pemetaan',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
                {
                    swalConfirm.fire('Berhasil Hapus Pemetaan!', data.pesan, 'success');
                    table.ajax.reload();
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
