...
function add_jadwal_rutin()
{
    ...
    $('#modal_title3').text('Tambahkan Jadwal Rutin'); 
}

function add_jadwal_tidak_rutin()
{
    reset_modal_form2();
    save_method = 'add';
    $('#modal_jadwal_tidak_rutin').modal('show');
    $('#modal_title2').text('Tambahkan Jadwal Tidak Rutin'); 
}
...
function reset_modal_form2()
{
    $('#form-jadwal-tidak-rutin')[0].reset();
    $('.append-opt').remove(); 
    $('div.form-group').children().removeClass("is-invalid invalid-feedback");
    $('span.help-block').text('');
    $('#div_pass_lama').css("display","none");
...
function save_jadwal_tidak_rutin()
{
    var url;
    var txtAksi;

    url = base_url + 'jadwal_dokter/add_jadwal_tidak_rutin';
    
    var form = $('#form-jadwal-tidak-rutin')[0];
    var data = new FormData(form);
...
function delete_jadwal_tidak_rutin(id){
    swalConfirmDelete.fire({
        title: 'Hapus Data ini ?',
        text: "Data Akan dihapus permanen ?",
        type: 'warning',
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'jadwal_dokter/delete_jadwal_tidak_rutin',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
