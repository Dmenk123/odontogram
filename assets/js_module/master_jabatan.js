...
$(document).ready(function() {
	...
	table = $('#tabel_index').DataTable({
        ...
        serverSide: true,
		ajax: {
			url  : base_url + "master_jabatan/list_jabatan",
			type : "POST" 
		},
...
function add_jabatan()
{
	...
	$('#modal_title').text('Tambah Jabatan Baru'); 
}
...
function edit_jabatan(id)
{
    ...
    $.ajax({
        url : base_url + 'master_jabatan/edit_jabatan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            $('[name="id_jabatan"]').val(data.old_data.id);
            $('[name="nama"]').val(data.old_data.nama);
            $('[name="keterangan"]').val(data.old_data.keterangan);
            $('#modal_form').modal('show');
	        $('#modal_title').text('Edit Data Jabatan'); 
        },
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_jabatan/add_data_jabatan';
        txtAksi = 'Tambah Jabatan';
    }else{
        url = base_url + 'master_jabatan/update_data_jabatan';
        txtAksi = 'Edit Jabatan';
    }
...
function delete_jabatan(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_jabatan/delete_jabatan',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
                {
                    swalConfirm.fire('Berhasil Hapus Jabatan!', data.pesan, 'success');
                    table.ajax.reload();
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
