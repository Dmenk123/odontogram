...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_role').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "set_role/list_role",
			type : "POST" 
		},
    ...
    $('.tombol-simpan').click(function(e){
        ...
        swalConfirm.fire({
            title: 'Simpan Pengaturan Hak Akses User ?',
    ...
    $(document).on('click', '.btn_edit_status', function(){
        ...
        var status = $(this).val();
        swalConfirm.fire({
            title: 'Ubah Status Data Hak Akses ?',
            text: "Apakah Anda Yakin ?",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.value) {
                $.ajax({
                    url : base_url + 'set_role/edit_status_role/'+ id,
                    type: "POST",
                    dataType: "JSON",
                    data : {status : status},
